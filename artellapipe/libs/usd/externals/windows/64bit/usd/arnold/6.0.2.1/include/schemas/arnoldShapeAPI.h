...
//
#ifndef USDARNOLD_GENERATED_ARNOLDSHAPEAPI_H
#define USDARNOLD_GENERATED_ARNOLDSHAPEAPI_H

/// \file usdArnold/arnoldShapeAPI.h
...
/// as the value.
///
class UsdArnoldShapeAPI : public UsdAPISchemaBase
{
public:
    ...
    /// Construct a UsdArnoldShapeAPI on UsdPrim \p prim .
    /// Equivalent to UsdArnoldShapeAPI::Get(prim.GetStage(), prim.GetPath())
    /// for a \em valid \p prim, but will not immediately throw an error for
    /// an invalid \p prim
    explicit UsdArnoldShapeAPI(const UsdPrim& prim=UsdPrim())
        : UsdAPISchemaBase(prim)
    {
    }

    /// Construct a UsdArnoldShapeAPI on the prim held by \p schemaObj .
    /// Should be preferred over UsdArnoldShapeAPI(schemaObj.GetPrim()),
    /// as it preserves SchemaBase state.
    explicit UsdArnoldShapeAPI(const UsdSchemaBase& schemaObj)
    ...
    /// Return a UsdArnoldShapeAPI holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// UsdArnoldShapeAPI(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    USDARNOLD_API
    static UsdArnoldShapeAPI
    Get(const UsdStagePtr &stage, const SdfPath &path);


    /// Applies this <b>single-apply</b> API schema to the given \p prim.
    /// This information is stored by adding "ArnoldShapeAPI" to the 
    /// token-valued, listOp metadata \em apiSchemas on the prim.
    /// 
    /// \return A valid UsdArnoldShapeAPI object is returned upon success. 
    /// An invalid (or empty) UsdArnoldShapeAPI object is returned upon 
