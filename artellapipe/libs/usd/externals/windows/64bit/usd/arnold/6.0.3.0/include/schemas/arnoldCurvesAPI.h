...
//
#ifndef USDARNOLD_GENERATED_ARNOLDCURVESAPI_H
#define USDARNOLD_GENERATED_ARNOLDCURVESAPI_H

/// \file usdArnold/arnoldCurvesAPI.h
...
/// as the value.
///
class UsdArnoldCurvesAPI : public UsdAPISchemaBase
{
public:
    ...
    /// Construct a UsdArnoldCurvesAPI on UsdPrim \p prim .
    /// Equivalent to UsdArnoldCurvesAPI::Get(prim.GetStage(), prim.GetPath())
    /// for a \em valid \p prim, but will not immediately throw an error for
    /// an invalid \p prim
    explicit UsdArnoldCurvesAPI(const UsdPrim& prim=UsdPrim())
        : UsdAPISchemaBase(prim)
    {
    }

    /// Construct a UsdArnoldCurvesAPI on the prim held by \p schemaObj .
    /// Should be preferred over UsdArnoldCurvesAPI(schemaObj.GetPrim()),
    /// as it preserves SchemaBase state.
    explicit UsdArnoldCurvesAPI(const UsdSchemaBase& schemaObj)
    ...
    /// Return a UsdArnoldCurvesAPI holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// UsdArnoldCurvesAPI(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    USDARNOLD_API
    static UsdArnoldCurvesAPI
    Get(const UsdStagePtr &stage, const SdfPath &path);


    /// Applies this <b>single-apply</b> API schema to the given \p prim.
    /// This information is stored by adding "ArnoldCurvesAPI" to the 
    /// token-valued, listOp metadata \em apiSchemas on the prim.
    /// 
    /// \return A valid UsdArnoldCurvesAPI object is returned upon success. 
    /// An invalid (or empty) UsdArnoldCurvesAPI object is returned upon 
...
public:
    // --------------------------------------------------------------------- //
    // PRIMVARSARNOLDORIENTATIONS 
    // --------------------------------------------------------------------- //
    /// 
    ///
    /// | ||
    /// | -- | -- |
    /// | Declaration | `vector3f[] primvars:arnold:orientations` |
    /// | C++ Type | VtArray<GfVec3f> |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Vector3fArray |
    USDARNOLD_API
    UsdAttribute GetPrimvarsArnoldOrientationsAttr() const;

    /// See GetPrimvarsArnoldOrientationsAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    USDARNOLD_API
    UsdAttribute CreatePrimvarsArnoldOrientationsAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `token primvars:arnold:basis = "bezier"` |
    /// | C++ Type | TfToken |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Token |
    /// | \ref UsdArnoldTokens "Allowed Values" | bezier, b-spline, catmull-rom, linear |
    USDARNOLD_API
    UsdAttribute GetPrimvarsArnoldBasisAttr() const;

    /// See GetPrimvarsArnoldBasisAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    USDARNOLD_API
    UsdAttribute CreatePrimvarsArnoldBasisAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `token primvars:arnold:mode = "ribbon"` |
    /// | C++ Type | TfToken |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Token |
    /// | \ref UsdArnoldTokens "Allowed Values" | ribbon, thick, oriented |
    USDARNOLD_API
    UsdAttribute GetPrimvarsArnoldModeAttr() const;

    /// See GetPrimvarsArnoldModeAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    USDARNOLD_API
    UsdAttribute CreatePrimvarsArnoldModeAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;

public:
    // --------------------------------------------------------------------- //
    // PRIMVARSARNOLDMIN_PIXEL_WIDTH 
    // --------------------------------------------------------------------- //
    /// 
    ///
    /// | ||
    /// | -- | -- |
    /// | Declaration | `float primvars:arnold:min_pixel_width = 0` |
    /// | C++ Type | float |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Float |
    USDARNOLD_API
    UsdAttribute GetPrimvarsArnoldMin_pixel_widthAttr() const;

    /// See GetPrimvarsArnoldMin_pixel_widthAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    USDARNOLD_API
    UsdAttribute CreatePrimvarsArnoldMin_pixel_widthAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `float2[] primvars:arnold:uvs` |
    /// | C++ Type | VtArray<GfVec2f> |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Float2Array |
    USDARNOLD_API
    UsdAttribute GetPrimvarsArnoldUvsAttr() const;

    /// See GetPrimvarsArnoldUvsAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    USDARNOLD_API
    UsdAttribute CreatePrimvarsArnoldUvsAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
