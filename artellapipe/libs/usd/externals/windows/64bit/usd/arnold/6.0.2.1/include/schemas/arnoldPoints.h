...
//
#ifndef USDARNOLD_GENERATED_ARNOLDPOINTS_H
#define USDARNOLD_GENERATED_ARNOLDPOINTS_H

/// \file usdArnold/arnoldPoints.h
...
/// as the value.
///
class UsdArnoldPoints : public UsdArnoldShape
{
public:
    ...
    /// Construct a UsdArnoldPoints on UsdPrim \p prim .
    /// Equivalent to UsdArnoldPoints::Get(prim.GetStage(), prim.GetPath())
    /// for a \em valid \p prim, but will not immediately throw an error for
    /// an invalid \p prim
    explicit UsdArnoldPoints(const UsdPrim& prim=UsdPrim())
        : UsdArnoldShape(prim)
    {
    }

    /// Construct a UsdArnoldPoints on the prim held by \p schemaObj .
    /// Should be preferred over UsdArnoldPoints(schemaObj.GetPrim()),
    /// as it preserves SchemaBase state.
    explicit UsdArnoldPoints(const UsdSchemaBase& schemaObj)
    ...
    /// Return a UsdArnoldPoints holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// UsdArnoldPoints(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    USDARNOLD_API
    static UsdArnoldPoints
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `float[] aspect` |
    /// | C++ Type | VtArray<float> |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->FloatArray |
    USDARNOLD_API
    UsdAttribute GetAspectAttr() const;

    /// See GetAspectAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    USDARNOLD_API
    UsdAttribute CreateAspectAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
