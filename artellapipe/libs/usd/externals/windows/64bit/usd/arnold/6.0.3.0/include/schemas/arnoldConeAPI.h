...
/// as the value.
///
class UsdArnoldConeAPI : public UsdAPISchemaBase
{
public:
...
public:
    ...
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Float |
    USDARNOLD_API
    UsdAttribute GetPrimvarsArnoldTop_radiusAttr() const;

    /// See GetPrimvarsArnoldTop_radiusAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    USDARNOLD_API
    UsdAttribute CreatePrimvarsArnoldTop_radiusAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
