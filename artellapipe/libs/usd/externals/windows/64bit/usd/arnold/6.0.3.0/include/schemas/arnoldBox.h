...
//
#ifndef USDARNOLD_GENERATED_ARNOLDBOX_H
#define USDARNOLD_GENERATED_ARNOLDBOX_H

/// \file usdArnold/arnoldBox.h
...
///
///
class UsdArnoldBox : public UsdArnoldShape
{
public:
    ...
    /// Construct a UsdArnoldBox on UsdPrim \p prim .
    /// Equivalent to UsdArnoldBox::Get(prim.GetStage(), prim.GetPath())
    /// for a \em valid \p prim, but will not immediately throw an error for
    /// an invalid \p prim
    explicit UsdArnoldBox(const UsdPrim& prim=UsdPrim())
        : UsdArnoldShape(prim)
    {
    }

    /// Construct a UsdArnoldBox on the prim held by \p schemaObj .
    /// Should be preferred over UsdArnoldBox(schemaObj.GetPrim()),
    /// as it preserves SchemaBase state.
    explicit UsdArnoldBox(const UsdSchemaBase& schemaObj)
    ...
    /// Return a UsdArnoldBox holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// UsdArnoldBox(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    USDARNOLD_API
    static UsdArnoldBox
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `vector3f min = (-0.5, -0.5, -0.5)` |
    /// | C++ Type | GfVec3f |
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `vector3f max = (0.5, 0.5, 0.5)` |
    /// | C++ Type | GfVec3f |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Vector3f |
    USDARNOLD_API
