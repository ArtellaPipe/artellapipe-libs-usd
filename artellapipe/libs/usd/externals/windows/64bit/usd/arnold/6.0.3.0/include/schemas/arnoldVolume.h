...
//
#ifndef USDARNOLD_GENERATED_ARNOLDVOLUME_H
#define USDARNOLD_GENERATED_ARNOLDVOLUME_H

/// \file usdArnold/arnoldVolume.h
...
/// \class UsdArnoldVolume
///
///
class UsdArnoldVolume : public UsdArnoldShape
{
public:
    ...
    /// Construct a UsdArnoldVolume on UsdPrim \p prim .
    /// Equivalent to UsdArnoldVolume::Get(prim.GetStage(), prim.GetPath())
    /// for a \em valid \p prim, but will not immediately throw an error for
    /// an invalid \p prim
    explicit UsdArnoldVolume(const UsdPrim& prim=UsdPrim())
        : UsdArnoldShape(prim)
    {
    }

    /// Construct a UsdArnoldVolume on the prim held by \p schemaObj .
    /// Should be preferred over UsdArnoldVolume(schemaObj.GetPrim()),
    /// as it preserves SchemaBase state.
    explicit UsdArnoldVolume(const UsdSchemaBase& schemaObj)
    ...
    /// Return a UsdArnoldVolume holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// UsdArnoldVolume(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    USDARNOLD_API
    static UsdArnoldVolume
