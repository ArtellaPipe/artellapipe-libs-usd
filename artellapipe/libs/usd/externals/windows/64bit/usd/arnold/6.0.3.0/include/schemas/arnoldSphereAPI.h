...
/// as the value.
///
class UsdArnoldSphereAPI : public UsdAPISchemaBase
{
public:
...
public:
    ...
    USDARNOLD_API
    UsdAttribute GetPrimvarsArnoldCenterAttr() const;

    /// See GetPrimvarsArnoldCenterAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    USDARNOLD_API
    UsdAttribute CreatePrimvarsArnoldCenterAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
