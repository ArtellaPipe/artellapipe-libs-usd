...
//
#ifndef USDARNOLD_GENERATED_ARNOLDPOINTSAPI_H
#define USDARNOLD_GENERATED_ARNOLDPOINTSAPI_H

/// \file usdArnold/arnoldPointsAPI.h
...
/// as the value.
///
class UsdArnoldPointsAPI : public UsdAPISchemaBase
{
public:
    ...
    /// Construct a UsdArnoldPointsAPI on UsdPrim \p prim .
    /// Equivalent to UsdArnoldPointsAPI::Get(prim.GetStage(), prim.GetPath())
    /// for a \em valid \p prim, but will not immediately throw an error for
    /// an invalid \p prim
    explicit UsdArnoldPointsAPI(const UsdPrim& prim=UsdPrim())
        : UsdAPISchemaBase(prim)
    {
    }

    /// Construct a UsdArnoldPointsAPI on the prim held by \p schemaObj .
    /// Should be preferred over UsdArnoldPointsAPI(schemaObj.GetPrim()),
    /// as it preserves SchemaBase state.
    explicit UsdArnoldPointsAPI(const UsdSchemaBase& schemaObj)
    ...
    /// Return a UsdArnoldPointsAPI holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// UsdArnoldPointsAPI(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    USDARNOLD_API
    static UsdArnoldPointsAPI
    Get(const UsdStagePtr &stage, const SdfPath &path);


    /// Applies this <b>single-apply</b> API schema to the given \p prim.
    /// This information is stored by adding "ArnoldPointsAPI" to the 
    /// token-valued, listOp metadata \em apiSchemas on the prim.
    /// 
    /// \return A valid UsdArnoldPointsAPI object is returned upon success. 
    /// An invalid (or empty) UsdArnoldPointsAPI object is returned upon 
...
public:
    // --------------------------------------------------------------------- //
    // PRIMVARSARNOLDASPECT 
    // --------------------------------------------------------------------- //
    /// 
    ///
    /// | ||
    /// | -- | -- |
    /// | Declaration | `float[] primvars:arnold:aspect` |
    /// | C++ Type | VtArray<float> |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->FloatArray |
    USDARNOLD_API
    UsdAttribute GetPrimvarsArnoldAspectAttr() const;

    /// See GetPrimvarsArnoldAspectAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    USDARNOLD_API
    UsdAttribute CreatePrimvarsArnoldAspectAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;

public:
    // --------------------------------------------------------------------- //
    // PRIMVARSARNOLDROTATION 
    // --------------------------------------------------------------------- //
    /// 
    ///
    /// | ||
    /// | -- | -- |
    /// | Declaration | `float[] primvars:arnold:rotation` |
    /// | C++ Type | VtArray<float> |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->FloatArray |
    USDARNOLD_API
    UsdAttribute GetPrimvarsArnoldRotationAttr() const;

    /// See GetPrimvarsArnoldRotationAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    USDARNOLD_API
    UsdAttribute CreatePrimvarsArnoldRotationAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `token primvars:arnold:mode = "disk"` |
    /// | C++ Type | TfToken |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Token |
    /// | \ref UsdArnoldTokens "Allowed Values" | disk, sphere, quad |
    USDARNOLD_API
