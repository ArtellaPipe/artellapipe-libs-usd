...
//
#ifndef USDARNOLD_GENERATED_ARNOLDDISKLIGHTAPI_H
#define USDARNOLD_GENERATED_ARNOLDDISKLIGHTAPI_H

/// \file usdArnold/arnoldDiskLightAPI.h
...
/// \class UsdArnoldDiskLightAPI
///
///
class UsdArnoldDiskLightAPI : public UsdAPISchemaBase
{
public:
    ...
    /// Construct a UsdArnoldDiskLightAPI on UsdPrim \p prim .
    /// Equivalent to UsdArnoldDiskLightAPI::Get(prim.GetStage(), prim.GetPath())
    /// for a \em valid \p prim, but will not immediately throw an error for
    /// an invalid \p prim
    explicit UsdArnoldDiskLightAPI(const UsdPrim& prim=UsdPrim())
        : UsdAPISchemaBase(prim)
    {
    }

    /// Construct a UsdArnoldDiskLightAPI on the prim held by \p schemaObj .
    /// Should be preferred over UsdArnoldDiskLightAPI(schemaObj.GetPrim()),
    /// as it preserves SchemaBase state.
    explicit UsdArnoldDiskLightAPI(const UsdSchemaBase& schemaObj)
    ...
    /// Return a UsdArnoldDiskLightAPI holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// UsdArnoldDiskLightAPI(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    USDARNOLD_API
    static UsdArnoldDiskLightAPI
    Get(const UsdStagePtr &stage, const SdfPath &path);


    /// Applies this <b>single-apply</b> API schema to the given \p prim.
    /// This information is stored by adding "ArnoldDiskLightAPI" to the 
    /// token-valued, listOp metadata \em apiSchemas on the prim.
    /// 
    /// \return A valid UsdArnoldDiskLightAPI object is returned upon success. 
    /// An invalid (or empty) UsdArnoldDiskLightAPI object is returned upon 
...
public:
    ...
    USDARNOLD_API
    UsdAttribute GetPrimvarsArnoldPositionAttr() const;

    /// See GetPrimvarsArnoldPositionAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    USDARNOLD_API
    UsdAttribute CreatePrimvarsArnoldPositionAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
