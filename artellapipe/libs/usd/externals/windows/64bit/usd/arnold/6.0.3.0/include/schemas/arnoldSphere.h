...
//
#ifndef USDARNOLD_GENERATED_ARNOLDSPHERE_H
#define USDARNOLD_GENERATED_ARNOLDSPHERE_H

/// \file usdArnold/arnoldSphere.h
...
/// \class UsdArnoldSphere
///
///
class UsdArnoldSphere : public UsdArnoldShape
{
public:
    ...
    /// Construct a UsdArnoldSphere on UsdPrim \p prim .
    /// Equivalent to UsdArnoldSphere::Get(prim.GetStage(), prim.GetPath())
    /// for a \em valid \p prim, but will not immediately throw an error for
    /// an invalid \p prim
    explicit UsdArnoldSphere(const UsdPrim& prim=UsdPrim())
        : UsdArnoldShape(prim)
    {
    }

    /// Construct a UsdArnoldSphere on the prim held by \p schemaObj .
    /// Should be preferred over UsdArnoldSphere(schemaObj.GetPrim()),
    /// as it preserves SchemaBase state.
    explicit UsdArnoldSphere(const UsdSchemaBase& schemaObj)
    ...
    /// Return a UsdArnoldSphere holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// UsdArnoldSphere(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    USDARNOLD_API
    static UsdArnoldSphere
