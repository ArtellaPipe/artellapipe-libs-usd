...
//
#ifndef USDARNOLD_GENERATED_ARNOLDMESHLIGHTAPI_H
#define USDARNOLD_GENERATED_ARNOLDMESHLIGHTAPI_H

/// \file usdArnold/arnoldMeshLightAPI.h
...
/// \class UsdArnoldMeshLightAPI
///
///
class UsdArnoldMeshLightAPI : public UsdAPISchemaBase
{
public:
    ...
    /// Construct a UsdArnoldMeshLightAPI on UsdPrim \p prim .
    /// Equivalent to UsdArnoldMeshLightAPI::Get(prim.GetStage(), prim.GetPath())
    /// for a \em valid \p prim, but will not immediately throw an error for
    /// an invalid \p prim
    explicit UsdArnoldMeshLightAPI(const UsdPrim& prim=UsdPrim())
        : UsdAPISchemaBase(prim)
    {
    }

    /// Construct a UsdArnoldMeshLightAPI on the prim held by \p schemaObj .
    /// Should be preferred over UsdArnoldMeshLightAPI(schemaObj.GetPrim()),
    /// as it preserves SchemaBase state.
    explicit UsdArnoldMeshLightAPI(const UsdSchemaBase& schemaObj)
    ...
    /// Return a UsdArnoldMeshLightAPI holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// UsdArnoldMeshLightAPI(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    USDARNOLD_API
    static UsdArnoldMeshLightAPI
    Get(const UsdStagePtr &stage, const SdfPath &path);


    /// Applies this <b>single-apply</b> API schema to the given \p prim.
    /// This information is stored by adding "ArnoldMeshLightAPI" to the 
    /// token-valued, listOp metadata \em apiSchemas on the prim.
    /// 
    /// \return A valid UsdArnoldMeshLightAPI object is returned upon success. 
    /// An invalid (or empty) UsdArnoldMeshLightAPI object is returned upon 
