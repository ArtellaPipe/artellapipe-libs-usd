...
//
#ifndef USDARNOLD_GENERATED_ARNOLDCURVES_H
#define USDARNOLD_GENERATED_ARNOLDCURVES_H

/// \file usdArnold/arnoldCurves.h
...
/// as the value.
///
class UsdArnoldCurves : public UsdArnoldShape
{
public:
    ...
    /// Construct a UsdArnoldCurves on UsdPrim \p prim .
    /// Equivalent to UsdArnoldCurves::Get(prim.GetStage(), prim.GetPath())
    /// for a \em valid \p prim, but will not immediately throw an error for
    /// an invalid \p prim
    explicit UsdArnoldCurves(const UsdPrim& prim=UsdPrim())
        : UsdArnoldShape(prim)
    {
    }

    /// Construct a UsdArnoldCurves on the prim held by \p schemaObj .
    /// Should be preferred over UsdArnoldCurves(schemaObj.GetPrim()),
    /// as it preserves SchemaBase state.
    explicit UsdArnoldCurves(const UsdSchemaBase& schemaObj)
    ...
    /// Return a UsdArnoldCurves holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// UsdArnoldCurves(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    USDARNOLD_API
    static UsdArnoldCurves
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `uint[] num_points` |
    /// | C++ Type | VtArray<unsigned int> |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->UIntArray |
    USDARNOLD_API
    UsdAttribute GetNum_pointsAttr() const;

    /// See GetNum_pointsAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    USDARNOLD_API
    UsdAttribute CreateNum_pointsAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `vector3f[] orientations` |
    /// | C++ Type | VtArray<GfVec3f> |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Vector3fArray |
    USDARNOLD_API
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `token basis = "bezier"` |
    /// | C++ Type | TfToken |
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `float min_pixel_width = 0` |
    /// | C++ Type | float |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Float |
    USDARNOLD_API
    UsdAttribute GetMin_pixel_widthAttr() const;

    /// See GetMin_pixel_widthAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    USDARNOLD_API
    UsdAttribute CreateMin_pixel_widthAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `float2[] uvs` |
    /// | C++ Type | VtArray<GfVec2f> |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Float2Array |
    USDARNOLD_API
    UsdAttribute GetUvsAttr() const;

    /// See GetUvsAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    USDARNOLD_API
    UsdAttribute CreateUvsAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
